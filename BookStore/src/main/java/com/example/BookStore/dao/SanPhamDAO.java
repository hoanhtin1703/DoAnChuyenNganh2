...
public class SanPhamDAO implements DAO<SanPham> {
    ...
    public BangSanPhamDTO timSanPhamTheoIdDeHienThi(int id) {
        TypedQuery<SanPham> sanPhamTypedQuery = entityManager.createQuery("SELECT sp FROM SanPham sp WHERE sp.maSanPham=:x ORDER BY sp.maSanPham DESC", SanPham.class);
    ...
    //    Lấy 5 sản phẩm giảm giá mới nhất
    public List<BangSanPhamDTO> laySanPhamGiamGiaMoi() {
        TypedQuery<SanPham> sanPhamTypedQuery = entityManager.createQuery("SELECT sp FROM SanPham sp WHERE sp.trangThai = 2 ORDER BY sp.maSanPham DESC", SanPham.class);
        ;
        sanPhamTypedQuery.setMaxResults(5);
        return sanPhamTypedQuery.getResultList()
                .stream()
    ...
    public List<BangSanPhamDTO> laySanPhamBanChay() {
        String sql = "SELECT dhct.sanPham " +
                "FROM DonHangChiTiet dhct " +
                "GROUP BY dhct.sanPham " +
                "ORDER BY SUM(dhct.soLuong) DESC";

        TypedQuery<SanPham> sanPhamTypedQuery = entityManager.createQuery(sql, SanPham.class);
        sanPhamTypedQuery.setMaxResults(5);
        return sanPhamTypedQuery.getResultList()
                .stream()
                .map(sanPham -> new BangSanPhamDTO(sanPham, -1))
...
//    Lấy tất cả sản phẩm giảm gá
    public List<BangSanPhamDTO> layTatCaSanPhamGiamGia() {
        TypedQuery<SanPham> sanPhamTypedQuery = entityManager.createQuery("SELECT sp FROM SanPham sp WHERE sp.trangThai = 2 ORDER BY sp.maSanPham DESC", SanPham.class);
        ;
        return sanPhamTypedQuery.getResultList()
                .stream()
