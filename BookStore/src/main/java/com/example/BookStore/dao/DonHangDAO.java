...
public class DonHangDAO implements DAO<DonHang> {
    ...
    public DonHang TimKiemDonHangTheoID(String id){
    ...
    public List<BangDonHangDTO> layDanhSachDonHangTheoNguoiDung(int id) {
        TypedQuery<DonHang> donHangTypedQuery = entityManager.createQuery("SELECT dh FROM DonHang dh WHERE dh.maKhachHang=:x", DonHang.class);
        donHangTypedQuery.setParameter("x", id);
        return donHangTypedQuery.getResultList()
                .stream()
                .map(donHang -> new BangDonHangDTO(donHang, 1))
    ...
    @Transactional
    public void xoaDonHangTheoID(String id){
        DonHang donHangToRemove = TimKiemDonHangTheoID(id);
        System.out.println(donHangToRemove);
